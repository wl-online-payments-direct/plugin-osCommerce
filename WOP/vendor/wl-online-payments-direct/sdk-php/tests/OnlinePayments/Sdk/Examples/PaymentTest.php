...
use common\modules\orderPayment\WOP\OnlinePayments\Sdk\Domain\CompanyInformation;
...
use common\modules\orderPayment\WOP\OnlinePayments\Sdk\Domain\LineItem;
use common\modules\orderPayment\WOP\OnlinePayments\Sdk\Domain\LineItemInvoiceData;
...
class PaymentTest extends ClientTestCase
{
    ...
    public function testCapturePayment($paymentId)
    {
        ...
        $captureResponse = $client->merchant($merchantId)->payments()->capturePayment($paymentId, $capturePaymentRequest);
    ...
    public function testCreateMinimalPayment()
    {
        ...
        $client = $this->getClient();
        $merchantId = $this->getMerchantId();
        $createPaymentResponse = $client->merchant($merchantId)->payments()->createPayment($this->getMinimalCreatePaymentRequest());
        return $createPaymentResponse->getPayment()->getId();
    }
    /**
     ...
     */
    protected function getMinimalCreatePaymentRequest($correctCreditCardNumber = \true)
    {
        $createPaymentRequest = new CreatePaymentRequest();
        $order = new Order();
        $amountOfMoney = new AmountOfMoney();
    ...
    public function testCreatePaymentWithIdempotenceKeyFailure()
    {
        ...
        $dateTimeWitMicroseconds = DateTime::createFromFormat('U.u', (string) microtime(\true));
        ...
        try {
            $client->merchant($merchantId)->payments()->createPayment($this->getMinimalCreatePaymentRequest(\false), $callContext);
