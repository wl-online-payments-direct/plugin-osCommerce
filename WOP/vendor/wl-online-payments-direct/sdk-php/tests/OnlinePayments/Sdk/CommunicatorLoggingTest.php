...
use stdClass;
use common\modules\orderPayment\WOP\OnlinePayments\Sdk\Communication\CommunicatorLoggerHelper;
...
use common\modules\orderPayment\WOP\OnlinePayments\Sdk\Communication\HttpObfuscator;
...
use common\modules\orderPayment\WOP\OnlinePayments\Sdk\TestUtil\TestErrorResponse;
use common\modules\orderPayment\WOP\OnlinePayments\Sdk\TestUtil\TestingAuthenticator;
use common\modules\orderPayment\WOP\OnlinePayments\Sdk\TestUtil\TestingConnection;
/**
 * @group logging
 */
class CommunicatorLoggingTest extends TestCase
{
    ...
    {
        $connection = new TestingConnection(new ConnectionResponse(200, array('Content-Type' => 'application/json'), '{}'));
        $communicator = new Communicator($this->getCommunicatorConfiguration(), new TestingAuthenticator(), $connection);
        $logger = $this->getMockBuilder('common\modules\orderPayment\WOP\OnlinePayments\Sdk\Logging\CommunicatorLogger')->getMock();
        $logger->expects($this->exactly(2))->method('log')->will($this->returnCallback(function ($message) {
    ...
    public function testLoggingForSuccessResponse()
    {
        ...
        $rawObfuscatedRequest = $httpObfuscator->getRawObfuscatedRequest('POST', $relativeRequestUriWithRequestParameters, $requestHeaders, $requestBody->toJson());
        $logger = $this->getMockBuilder('common\modules\orderPayment\WOP\OnlinePayments\Sdk\Logging\CommunicatorLogger')->getMock();
        $logger->expects($this->exactly(2))->method('log')->will($this->returnCallback(function ($message) use ($rawObfuscatedRequest) {
    ...
    {
        $relativeRequestUri = '/foo/bar';
        $connection = new TestingConnection(new ConnectionResponse(200, array('Content-Type' => 'application/json;charset=UTF-8'), '{}'));
        $communicator = new Communicator($this->getCommunicatorConfiguration(), new TestingAuthenticator(), $connection);
        $relativeRequestUriWithRequestParameters = $relativeRequestUri;
        $requestHeaders = $this->getCommunicatorRequestHeaders($communicator, 'POST', $relativeRequestUriWithRequestParameters);
        $requestBody = $this->getMockRequestDataObject();
    ...
    public function testLoggingForClientErrorResponse()
    {
        ...
        $logger = $this->getMockBuilder('common\modules\orderPayment\WOP\OnlinePayments\Sdk\Logging\CommunicatorLogger')->getMock();
        $logger->expects($this->exactly(2))->method('log')->will($this->returnCallback(function ($message) use ($rawObfuscatedResponse) {
            $messageHeader = strstr($message, "\n", \true);
        ...
        $communicator->enableLogging($logger);
        $responseClassMap = new ResponseClassMap();
        $responseClassMap->defaultErrorResponseClassName = 'common\modules\orderPayment\WOP\OnlinePayments\Sdk\TestUtil\TestErrorResponse';
        try {
    ...
    public function testLoggingForCommunicationException()
    {
        ...
        $communicator = new Communicator($this->getCommunicatorConfiguration(), new TestingAuthenticator(), $connection);
        $logger = $this->getMockBuilder('common\modules\orderPayment\WOP\OnlinePayments\Sdk\Logging\CommunicatorLogger')->getMock();
        $logger->expects($this->once())->method('log')->will($this->returnCallback(function ($message) {
            $messageHeader = strstr($message, "\n", \true);
            $this->assertStringContainsString('Outgoing request', $messageHeader);
        }));
        $logger->expects($this->once())->method('logException')->will($this->returnCallback(function ($message, $exception) use ($errorException) {
    ...
    {
        $requestDataObject = $this->getMockBuilder('common\modules\orderPayment\WOP\OnlinePayments\Sdk\Domain\DataObject')->getMock();
     ...
     * @return string[]
     */
    protected function getCommunicatorRequestHeaders(CommunicatorInterface $communicator, $httpMethod, $relativeUriPathWithRequestParameters, $clientMetaInfo = '', ?CallContext $callContext = null)
    {
        $method = new ReflectionMethod($communicator, 'getRequestHeaders');
        $method->setAccessible(\true);
        return $method->invoke($communicator, $httpMethod, $relativeUriPathWithRequestParameters, Communicator::MIME_APPLICATION_JSON, $clientMetaInfo, $callContext);
    }
}
